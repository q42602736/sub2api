...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		accountReleaseFunc, acquired := h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, selection, parsed.Stream, &streamStarted, preserveStickyBinding, reqLog)
		if !acquired {
			return
		}
