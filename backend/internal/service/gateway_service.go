...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	...
	if !hasAnyMapping {
		if platform == PlatformOpenAI {
			models := filterDefaultOpenAIModelsByAccounts(accounts)
			if s.modelsListCache != nil {
...
func filterDefaultOpenAIModelsByAccounts(accounts []Account) []string {
	ids := openai.DefaultModelIDs()
	filtered := make([]string, 0, len(ids))
	for _, modelID := range ids {
		if IsOpenAIDefaultModelSupportedByAnyAccount(modelID, accounts) {
...
// IsOpenAIDefaultModelSupportedByAnyAccount 判断默认 OpenAI 模型是否应对当前账号池暴露。
func IsOpenAIDefaultModelSupportedByAnyAccount(modelID string, accounts []Account) bool {
	if len(accounts) == 0 {
		return true
	}
	for i := range accounts {
