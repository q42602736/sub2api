...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64) *Account {
	...
	if !s.isOpenAIAccountSelectable(account, requestedModel, settings) ||
		!isOpenAIAccountEligibleForRequest(ctx, account, requestedModel, false) {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*Account, bool) {
	...
	if settings.enabled && settings.parallelEnabled {
		return s.selectRandomAccountWithinTopPriority(ctx, groupID, accounts, requestedModel, excludedIDs, requireCompact)
	}
...
func (s *OpenAIGatewayService) selectRandomAccountWithinTopPriority(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*Account, bool) {
	needsUpstreamCheck := s.needsUpstreamChannelRestrictionCheck(ctx, groupID)
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, account *Account, requestedModel string, requireCompact bool) *Account {
	...
	if !s.isOpenAIAccountSelectable(fresh, requestedModel, settings) ||
		!isOpenAIAccountEligibleForRequest(ctx, fresh, requestedModel, requireCompact) {
		return nil
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, requestedModel string, requireCompact bool) *Account {
	...
	if s.schedulerSnapshot == nil || s.accountRepo == nil {
		settings := s.getOpenAIOverLimitModeSettings(ctx)
		if !s.isOpenAIAccountSelectable(account, requestedModel, settings) ||
			!isOpenAIAccountEligibleForRequest(ctx, account, requestedModel, requireCompact) {
			return nil
		}
		if s.isOpenAIAccountRuntimeBlocked(account) {
			return nil
	...
	if !s.isOpenAIAccountSelectable(latest, requestedModel, settings) ||
		!isOpenAIAccountEligibleForRequest(ctx, latest, requestedModel, requireCompact) {
		return nil
	}
